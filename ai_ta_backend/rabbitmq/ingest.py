...
class Ingest:
    ...
    def split_and_upload(self, texts: List[str], metadatas: List[Dict[str, Any]], force_embeddings: bool, **kwargs):
        ...
        try:
            ...
            document_name = contexts[0].metadata.get('readable_filename') or os.path.basename(blob_path)
            document_type = document_name.split('.')[-1].lower() if '.' in document_name else 'unknown'
            groups = kwargs.get('groups', [])

            logging.info(f"?? Updating database for {blob_path}")

            # ? ONLY UPDATE contexts, NEVER touch metadata columns
            self.sql_session.update_document_contexts(
                blob_path=blob_path,
                contexts=contexts_for_supa
            )

            # ? Handle doc_group mappings (if needed)
            if groups:
                try:
                    if url:
                    ...
                    if count == 0:
                        logging.warning(f"?? Could not add document to groups: {groups}")
                except Exception as e:
                    logging.error(f"? Error adding to doc groups: {e}")
