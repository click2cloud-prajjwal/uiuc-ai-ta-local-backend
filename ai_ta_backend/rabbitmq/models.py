...
class Document(Base):
    ...
    readable_filename = Column(Text)  # Keep for backward compatibility
    document_name = Column(Text)      # ? ADD THIS
    document_type = Column(Text)      # ? ADD THIS
    course_name = Column(Text)
    url = Column(Text)
    contexts = Column(JSON, default=lambda: [])
    base_url = Column(Text)
    doc_groups = Column(ARRAY(Text))  # ? ADD THIS
    ...
    def to_dict(self):
        return {
            ...
            "document_name": self.document_name,      # ? ADD
            "document_type": self.document_type,      # ? ADD
            "course_name": self.course_name,
            "url": self.url,
            "contexts": self.contexts,
            "base_url": self.base_url,
            "doc_groups": self.doc_groups,           # ? ADD
        }


# ---------------------------------------------------------------------
# ?? Track In-Progress / Failed Ingest Jobs
# ---------------------------------------------------------------------
...
# ---------------------------------------------------------------------
# ??? Doc Groups (stores document group metadata)
# ---------------------------------------------------------------------
class DocGroup(Base):
