...
class SQLAlchemyIngestDB:
    ...
    def document_exists(self, blob_path: str) -> bool:
      with self.get_session() as session:
    ...
    def insert_document_metadata(
        ...
        """Insert document metadata - handles PostgreSQL array type"""
        with self.get_session() as session:
            session.execute(
                text("""
                ...
                """),
                {
                    ...
                    "doc_groups": doc_groups,  # PostgreSQL will handle list → array
                    "course_name": course_name,
                    "url": url,
                    "base_url": base_url,
                }
            )
            logging.info(f"✅ Document metadata inserted for {blob_path}")

    def update_document_contexts(self, blob_path: str, contexts):
      import json
      with self.get_session() as session:
          result = session.execute(
              text("""
                  UPDATE documents
                  SET contexts = CAST(:contexts AS json)
                  WHERE blob_path = :blob_path
                  RETURNING id, readable_filename, doc_groups
              """),
              {
